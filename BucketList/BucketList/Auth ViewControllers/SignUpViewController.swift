...
//            userController.signUp(with: user) { (error) in
            userController.signUp(name: name, email: email, password: password) { (error) in
                 if let error = error {
                     NSLog("Error occured during sign up: \(error)")
                 } else {
                     DispatchQueue.main.async {
                         let alertController = UIAlertController(title: "Sign Up Successful", message: "Welcome to Bucket List", preferredStyle: .alert)
